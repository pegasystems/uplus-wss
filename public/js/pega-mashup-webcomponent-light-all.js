...
/**
    ...
    ${e.values.map((t=>T` <option value=${t.value}>${t.name}</option>`))}
        ...
        `))}
    `}}customElements.define("rich-text-editor",Oe);const Pe=(t,e)=>{if(t.helperText&&""!==t.helperText)return`${e}-info`},He=t=>{if(t.placeholder&&""!==t.placeholder)return t.displayplaceholder},Fe=(t,e,a,i)=>void 0===e||""===e?T`
      ${i}
    `:T`
...
`,Xe=(t,e)=>new Oe(ct(t.displayvalue),t.reference,e),Qe=(t,e)=>{const a=t.requiredstate?"icon-required":"";return T`
  <input
  data-ref="${t.reference}"
  ?required="${t.requiredstate}"
  ?readonly="${t.readonlystate}"
