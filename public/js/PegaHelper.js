const standardHeight = '550' + 'px';
let initialMessageBoxDivHeight;
let adjustMessageFieldWidthInterval;
let previewLoaded = false;
let previewMonitor;
let MonitorTimeout = 2000;
let missedMessageCounter = 0;
let minimized = true;
const chatSessionStatusKey = 'chat-sesstion-connected';
let BotActive = false;
...
$(document).ready(() => {
  ...
  let $minL = $('<div>', { id: 'launcherminimized', text: 'Need Help?' });
  $minL.click(maximizeAdvisorWindow);
  let $counter = $('<div>', { id: 'unreadCounter', text: '0' });
  $minL.append($counter);
  $('body').append($minL);
...
function postMessageListener(event) {
  ...
  if (
    message.message &&
    message.message.payload.name == 'loaded' &&
    message.message.src == 'PreviewGadgetIfr'
  ) {
    ...
    /* enable launcher now */
    if (BotActive != true) {
      const $launcher = $('<div>', { id: 'launcher', text: 'Need Help?' });
...
function adjustMessageFieldWidth() {
  const width = $('#sendMessageField').width();
  if (width != null) {
...
function InvokeAdvisor(event, workId, queueName) {
  ...
  $('#launcher').hide();
  const PegaAParamObject = preparePegaAParams('OnlineHelp');
  if (workId) {
...
function monitorPreviewLoader() {
  if (previewLoaded == false) {
    const PegaAParamObject = preparePegaAParams('PreviewGadget');
    pega.web.api.doAction(
      'PreviewGadget',
      'display',
      'Preview',
      'PegaCS-OnlineHelp-Triage-WebChatbot',
      null,
      true,
      null,
      PegaAParamObject,
...
window.proactiveChatCustomEventHandler = function(gadgetName, customData) {
  ...
  const message = JSON.parse(customData);
  if (message.command == 'ProactiveChatStatus') {
    if (message.status === 'loaded') {
      ...
      InvokeAdvisor(
        null,
        message.WorkId,
        window.pega.chat.proactiveChat.queueName,
      );
...
PegaUtilEventListener.prototype.on = function(
  ...
  this.listenerStore[eventName].push({ fn: eventListener, context });
  return this;
};
PegaUtilEventListener.prototype.off = function(eventName, eventListener) {
  this.listenerStore[eventName] = this.listenerStore[eventName] || [];
  const listeners = this.listenerStore[eventName];
...
PegaUtilEventListener.prototype.trigger = function(eventName) {
  ...
  let listeners = this.listenerStore[eventName];
...
PegaProactiveChat.prototype.offer = function(queueName, metadata, bDefaultUI) {
  ...
  this.defaultUI = bDefaultUI !== false;
  metadata = metadata || {};
  const PegaAParamObject = preparePegaAParams('OnlineHelp');
  PegaAParamObject.queueName = queueName;
  PegaAParamObject.metadata = JSON.stringify(
    convertProactiveMetadata(metadata),
  );
  PegaAParamObject.hideDefaultUI = true;
  pega.web.api.doAction(
    'ProactiveChat',
    'createNewWork',
    'PegaCA-Work-ProactiveChat',
    'pyStartCase',
    PegaAParamObject,
  );
};
...
$(() => {
  $('body').append(
    `<div id='OnlineHelp' data-pega-gadgetname ='OnlineHelp' data-pega-action ='createNewWork' data-pega-action-param-classname ='' data-pega-action-param-flowname ='' data-pega-action-param-model ='' data-pega-applicationname ='${
      PegaCSWSS.ApplicationName
    }' data-pega-isdeferloaded ='true' data-pega-threadname ='CSAdvisor' data-pega-systemid ='pega' data-pega-resizetype ='fixed' data-pega-url ='${
      PegaCSWSS.MashupURL
    }' data-pega-action-param-parameters ='' data-pega-redirectguests='true' data-pega-event-onclose ='hideinline'></div>`,
  );
  $('body').append(
    `<div id='ProactiveChat' data-pega-gadgetname ='ProactiveChat' data-pega-action ='createNewWork' data-pega-action-param-classname ='' data-pega-action-param-flowname ='' data-pega-action-param-model ='' data-pega-applicationname ='${
      PegaCSWSS.ApplicationName
    }' data-pega-isdeferloaded ='true' data-pega-threadname ='ProactiveChatThread' data-pega-systemid ='pega' data-pega-resizetype ='fixed' data-pega-url ='${
      PegaCSWSS.MashupURL
    }' data-pega-action-param-parameters ='' data-pega-redirectguests='true' data-pega-event-onclose ='hideinline' data-pega-event-oncustom='proactiveChatCustomEventHandler'></div>`,
  );
  $('body').append(
    `<div style='display:none; height:80px; padding-top:14px; padding-left:20px;' id='Preview' data-pega-gadgetname ='PreviewGadget' data-pega-isdeferloaded ='true' data-pega-action ='display' data-pega-action-param-classname ='PegaCS-OnlineHelp-Triage-WebChatbot'  data-pega-harnessname ='Preview' data-pega-applicationname ='${
      PegaCSWSS.ApplicationName
    }' data-pega-threadname ='Preview' data-pega-systemid ='pega' data-pega-resizetype ='fixed' data-pega-event-onclose ='' data-pega-url ='${
      PegaCSWSS.MashupURL
